    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        myapp = { poetry2nix, lib }: poetry2nix.mkPoetryApplication {
          projectDir = self;
          overrides = poetry2nix.overrides.withDefaults (final: super: {
            chromedriver-autoinstaller = super.chromedriver-autoinstaller.overridePythonAttrs (old: {
              nativeBuildInputs = (old.nativeBuildInputs or []) ++ [ final.setuptools ];
            });

            jiter = super.jiter.overridePythonAttrs (old: {
                nativeBuildInputs = (old.nativeBuildInputs or []) ++ [ final.maturin ];
            });
          });
        };
